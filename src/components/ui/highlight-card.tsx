...
const HighlightCard: FC<ComponentProps> = ({ title, description, icon, className }) => {
  return (
    <div className="group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:-rotate-1">
      <Card className={cn("text-white rounded-2xl border border-white/10 bg-gradient-to-br from-[#010101] via-[#090909] to-[#010101] shadow-2xl relative backdrop-blur-xl overflow-hidden hover:border-white/25 hover:shadow-white/5 w-[350px]", className)}>
        <div className="absolute inset-0 z-0 overflow-hidden">
          ...
          <div className="absolute bottom-16 right-16 w-12 h-12 rounded-full bg-white/5 blur-lg animate-ping" />
        ...
        <div className="p-8 relative z-10 flex flex-col items-center text-center">
          ...
          <div className="space-y-1 max-w-sm">
            {description.map((line, idx) => (
              <p key={idx} className="text-gray-300 text-sm leading-relaxed transform group-hover:text-gray-200 transition-colors duration-300">
                {line}
              </p>
            ))}
          </div>
...
/**
 * Same highlight-card shell, but wrapping arbitrary portal content.
 * Used to give every card-shaped container across the app the identical look.
 * variant="feature" adds the hover lift/tilt + sweeping sheen.
 * variant="panel" keeps the surface calm for forms, editors and lists.
 */
export const HighlightPanel: FC<{
  children: ReactNode;
  className?: string;
...
}> = ({ children, className, variant = "feature", onClick, style, innerClassName }) => (
  <div
    onClick={onClick}
    style={style}
    className={cn(
      "group relative transform transition-all duration-500",
      variant === "feature" && "hover:scale-[1.02] hover:-rotate-[0.4deg]",
      onClick && "cursor-pointer",
      className,
    )}
  >
    <Card className="h-full text-white rounded-2xl border border-white/10 bg-gradient-to-br from-[#010101] via-[#090909] to-[#010101] shadow-2xl relative backdrop-blur-xl overflow-hidden transition-colors duration-500 hover:border-white/25">
